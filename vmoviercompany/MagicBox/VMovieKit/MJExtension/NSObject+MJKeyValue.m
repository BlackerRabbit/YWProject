...
#warning handle @[@[]] 
            if ([value isKindOfClass:[NSArray class]]) {
                if([[value lastObject] isKindOfClass:[NSArray class]]) {
...
- (NSDictionary*)keyValues
{
    ...
    [[self class] enumerateIvarsWithBlock:^(MJIvar* ivar, BOOL* stop) {
        ...
        } else if (typeClass == [NSMutableArray class] || typeClass == [NSArray class]) {
            //针对本app的情况 添加
            if ([value count] != 0) {
                value = [[value[0] class] keyValuesArrayWithObjectArray:value];
            }
        }
