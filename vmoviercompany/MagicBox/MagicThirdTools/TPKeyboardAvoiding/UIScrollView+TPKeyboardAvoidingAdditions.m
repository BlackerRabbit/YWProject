...
@property (nonatomic, assign) CGFloat      exterHeight;
...
- (void)TPKeyboardAvoiding_keyboardDidShow:(NSNotification*)notification {
    CGRect keyboardRect = [self convertRect:[[[notification userInfo] objectForKey:_UIKeyboardFrameEndUserInfoKey] CGRectValue] fromView:[UIApplication sharedApplication].keyWindow];
    keyboardRect = [[[notification userInfo] objectForKey:@"UIKeyboardFrameEndUserInfoKey"] CGRectValue];
    NSLog(@"the fucking da mo wang is %@",notification.userInfo);
    ...
    //NSLog(@"the fucko=ing keyboradRect is the fucking %@",NSStringFromCGRect(state.keyboardRect));
...
//    NSLog(@"contentInset\n  top = %f\n bottmo = %f \n left = %f\n ,right = %f",self.contentInset.top,self.contentInset.bottom,self.contentInset.left,self.contentInset.right);
    if ( firstResponder ) {
        CGFloat viewableHeight = self.bounds.size.height - self.contentInset.top - self.contentInset.bottom;
//        NSLog(@"size.height is %f /n self.contentInset.top is %f, self.contentInset.bottom is %f",self.bounds.size.height , self.contentInset.top , self.contentInset.bottom);
        
//         NSLog(@"TPKeyboardAvoiding_keyboardWillShow viewableHeight is %f",viewableHeight);
...
//    NSLog(@"TPKeyboardAvoiding_scrollToActiveTextField visibleSpace is %f",visibleSpace);
...
//    NSLog(@"最终大魔王来啦  %f",viewAreaHeight);
   ...
   // NSLog(@"offset11 is ===== >>> %f",offset);
    ...
    offset += state.exterHeight;
    //NSLog(@"offset33 is ===== >>> %f",offset);
    return offset;
}
...
-(void)moreExternHeight:(CGFloat)height{
    TPKeyboardAvoidingState *state = self.keyboardAvoidingState;
    state.exterHeight = height;
}
